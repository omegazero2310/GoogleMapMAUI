...
namespace GGMapMAUI.Platforms.Android.Logics
{
    public sealed class CameraLogic : BaseCameraLogic<GoogleMap>
    {
        ...
        public override void Register(Maui.GoogleMaps.Map map, GoogleMap nativeMap)
        {
        ...
        {
            var update = m.Update as Maui.GoogleMaps.CameraUpdate;
            MoveCamera(update);
            m.Callback.OnFinished();
        }

        internal void MoveCamera(Maui.GoogleMaps.CameraUpdate update)
        {
            _nativeMap.MoveCamera(update.ToAndroid(this.ScaledDensity));
        }
